...
用户对某条候选不满意,给出了修改指令。
指令可能是:
- 补充真实背景（如"其实我想答应但不想太主动""他是我领导"）→ 据此调整回复策略和措辞
- 调整方向（如"算了直接拒绝吧""加一句约他周末"）→ 改变回复的意图
- 微调语气/长度（如"再简短些""正式一点"）→ 保持意图不变,调格式
...
【用户补充的真实背景】
（聊天记录里看不出来、但影响回复策略的上下文——可能是现实中发生的事、双方真实关系、用户内心的真实意图等）
{custom_prompt or "(用户未补充)"}
