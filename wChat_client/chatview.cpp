    ...
    ,isAppended(false)
{
    ...
    pHLayout_1->addWidget(new QWidget(),10000);
    w->setLayout(pHLayout_1);
    m_pScrollArea->setWidget(w);

    m_pScrollArea->setVerticalScrollBarPolicy(Qt::ScrollBarAlwaysOff);//禁用了默认的垂直滚动条显示
...
void ChatView::appendChatItem(QWidget *item)
{
    ...
    //布局最后一个元素是 “高权重伸缩项”，插入到倒数第二位置可确保新消息在伸缩项上方
    vl->insertWidget(vl->count()-1,item);// 会触发QScrollBar::rangeChanged信号
    isAppended =true;
}
