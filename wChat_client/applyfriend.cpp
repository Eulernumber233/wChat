    ...
    ui(new Ui::ApplyFriend),_label_point(2,6)
{
    ...
    this->setModal(true);
    ui->certification->setPlaceholderText(tr("你好新朋友"));
    ...
    ui->sure_btn->SetState("normal","hover","press");
    //连接确认和取消按钮的槽函数
    connect(ui->sure_btn, &QPushButton::clicked, this, &ApplyFriend::SlotApplyCancel);
...
void ApplyFriend::SetSearchInfo(std::shared_ptr<SearchInfo> si)
{
    ...
    ui->certification->setText(applyname);
...
void ApplyFriend::SlotApplySure()
{
    ...
    jsonObj["uid"] = uid;
    auto certification = ui->certification->text();
    if(certification.isEmpty()){
        certification = ui->certification->placeholderText();
    }

    jsonObj["certification"] = certification;
