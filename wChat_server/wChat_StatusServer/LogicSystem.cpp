...
LogicSystem::LogicSystem()
{
	RegGet("/get_test", [](std::shared_ptr<HttpConnection>connection) {
		...
		for (auto& element : connection->_get_params) {
			i++;
			beast::ostream(connection->_response.body()) << "param" << i << "key is 哈哈哈哈" << element.first;
			beast::ostream(connection->_response.body()) << "param" << i << "key is 呵呵呵呵" << element.second << std::endl;
		}
		});
	...
	RegPost("/get_varifycode", [](std::shared_ptr<HttpConnection>connection) {
		...
		if (!parse_success) {
			std::cout << "Failed to parse JSON datal!" << std::endl;
    ...
    RegPost("/user_register", [](std::shared_ptr<HttpConnection> connection) {
        ...
        std::string icon = head_photo_head + std::to_string(randomNumber) + head_photo_tail;
