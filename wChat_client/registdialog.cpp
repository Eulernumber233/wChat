...
//"http://localhost:8080/get_varifycode"
void RegistDialog::slot_reg_mod_finish(ReqId id, QString res, ErrorCodes err)
{
    ...
    // 解析json字符串, res 转化为QString
    QJsonDocument jsonDoc =QJsonDocument::fromJson(res.toUtf8());
    if(jsonDoc.isNull()){
        showTip(tr("json解析失败_1"),false);
        return;
    }
    if(!jsonDoc.isObject()){
        showTip(tr("json解析失败_2"),false);
        return;
    }
    jsonDoc.object();
...
void RegistDialog::initHttpHandlers()
{
    _handlers.insert(ReqId::ID_GET_VARIFY_CODE,[this](const QJsonObject& jsonObj){
        ...
        email_const =jsonObj["email"].toString();
        showTip(tr("验证码已经发送到邮箱，注意查收"),true);
        qDebug()<<"email is "<<email_const<<Qt::endl;
    });
    _handlers.insert(ReqId::ID_REG_USER, [this](QJsonObject jsonObj){
        ...
        ChangerTipPage();
        qDebug()<< "user uuid is " << jsonObj["uuid"].toString() << Qt::endl;
...
void RegistDialog::ChangerTipPage(){
    _countdown_timer->stop();
...
bool RegistDialog::checkUserValid()
{
    ...
    if(isAllDigit(ui->user_edit->text())){
        AddTipErr(TipErr::TIP_USER_ERR, tr("用户名不能全为数字"));
        return false;
    }
...
void RegistDialog::on_ret_btn_clicked()
{
    _countdown_timer->stop();
    emit sigSwitchLogin();
}

bool RegistDialog::isAllDigit(const QString& str)
{
    for (QChar c : str) {
        if (c < '0' || c > '9') {
            return false;
